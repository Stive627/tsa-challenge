    ...
    <div  className='w-full flex justify-center mt-4'>
      <div className=' w-11/12 lg:w-1/4'>
        ...
        <div className=' flex flex-col gap-3 my-4'>
          {question?.suggestions.map((elt, indx) => <button key={indx} onClick={()=>select(indx)} style={{borderColor:'rgba(157, 78, 221, 1)', color:indx === currSuggestion ? ' white':'rgba(157, 78, 221, 1)', backgroundColor:indx === currSuggestion?'rgba(157, 78, 221, 1)':'transparent'}} className=' py-1 border rounded-xl'><p>{alphabet[indx]} {elt}</p></button>)}
        </div>
